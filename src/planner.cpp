...
vector<vector<double>> Planner::generate_kl_trajectory(Car car, 
    ...
    if (prev_path_size >= 2) {  // use last 2 points as anchor points
        ...
        double car_prev_x = car.x - cos(car.yaw) * prev_point_dist;
        double car_prev_y = car.y - sin(car.yaw) * prev_point_dist; 
        ...
        spline_pts_x.push_back(car.x);
        spline_pts_y.push_back(car.y);
    }
